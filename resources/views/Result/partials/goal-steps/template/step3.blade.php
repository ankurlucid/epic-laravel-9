...
<script type="text/javascript">
   ...
   function loadFormData(){
      ...
      $.ajax({
         ...
         success: function (data) {
            ...
            if(data.data && data.data.gb_goal_name_other != undefined && data.data.gb_goal_name_other != ""){
               ...
               $('#name_goal_other').text(data.data.gb_goal_name_other);
            }
         }
      });
   }
   ...
   function templateLoad(){
      ...
      for (var i = 0; i < goal_name.length; i++) {
         ...
         if (goal_name[i].trim() == "Other") {
            goal_name[i] = goal_name[i].trim();
            var other1 = "name_goal_other";
            if (goal_other != null) {
               html1 =
                  '<textarea rows="7" class="form-control" id="name_goal_other" name="gb_goal_name_other">' +
                  goal_other +
                  "</textarea>";
            } else {
               html1 =
                  '<textarea rows="7" class="form-control hidden" id="name_goal_other" name="gb_goal_name_other"></textarea>';

                  // '<textarea rows="7" class="form-control hidden" id="name_goal_other" name="gb_goal_name_other" placeholder="Input Your Specific Goal Here..."></textarea>';
            }
         }
