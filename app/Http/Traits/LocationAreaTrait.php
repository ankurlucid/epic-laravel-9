    ...
    protected function staffs($areaId){
        ...
        if($areaId == 'all'){
            if(isUserType(['Admin']) && Auth::user()->hasPermission(Auth::user(), 'list-staff')){
                $staffIdsObj = DB::table('area_staffs')->where('as_business_id', Session::get('businessId'))->whereNull('deleted_at')->distinct()->select('as_staff_id')->get();
            ...
            if(Auth::user()->hasPermission(Auth::user(), 'list-staff')){
                ...
                $area = LocationArea::findArea($areaId);
                if($area)
                    $staffs = $area->staffs;
            }
