...
<div class="joms-stream__header" id="post-comment-{{ $comment->id }}">                                         			
			...
			<div class="comments">
				...
				<a href="javascript:;">
					<span class="joms-stream__time" style="font-size: 16px;">
						<small id="comment-value-{{$comment->id}}"> {!! $comment->comment !!} </small>
					</span>
				</a>
			</div>
		</div>
 <!-- delete button --> 
   @if($post->client_id == Auth::user()->account_id || $comment->client_id == Auth::user()->account_id)
		<div class="text-align-right pr-0 dropdown delete-dropdown comment-delete" id="delete-option-{{$comment->id}}">
